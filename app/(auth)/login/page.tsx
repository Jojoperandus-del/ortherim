...
export default function LoginPage() {
  ...
  return (
    <div className="animate-fade-up">
      <div className="mb-8">
        <h1 className="text-2xl font-bold text-ink font-display">Bon retour 👋</h1>
      ...
      <form onSubmit={handleSubmit} className="space-y-4">
        ...
        <div>
          <div className="flex items-center justify-between mb-1.5">
            ...
            <Link href="#" className="text-xs text-brand-green hover:text-brand-green-dark transition-colors font-medium">
              Mot de passe oublié ?
            </Link>
          </div>
        ...
        {error && (
          <div className="bg-red-50 border border-red-200 rounded-xl px-4 py-3 text-red-700 text-sm flex items-center gap-2">
            <span className="w-4 h-4 rounded-full bg-red-200 flex items-center justify-center text-red-600 text-xs font-bold shrink-0">!</span>
            {error}
          </div>
        )}
      ...
      <div className="mt-6 pt-6 border-t border-surface-border text-center">
        <p className="text-sm text-ink-muted">
          ...
          <Link href="/register" className="text-brand-green font-semibold hover:text-brand-green-dark transition-colors">
            Créer un compte
          </Link>
        </p>
      </div>
